...
5. 法宝命名约束：法宝是经过灵气淬炼的修仙灵器，名称必须体现珍稀或灵异之感，禁止使用凡俗日用品名称（如布衣、草鞋、铁剑、破刀）。正确示例：青锋剑、灵丝道袍、玄铁护甲、紫电飞剑；错误示例：布衣、精刚剑、粗布劲装。
...
5. intro 规则：只描述功法的来历、传承渊源、外观形态、修炼时的意境感受。禁止在 intro 中使用"体修""法修""灵力""劲力""体魄"等体系或属性分类名词，也不得描述功法的具体功能效果。应采用修仙世界的玄学化描述风格，如"据传源自上古力修一脉，修炼时周身气血如山岳崩裂"。
6. system（体系）字段：只能从以下两种中选择一个：体修、法修。体系须与功法名称和描述契合。
7. role（定位）字段：只能从"攻击"和"辅助"中选择一个。攻击类功法（如崩山诀、烈火术）必须选择"攻击"，辅助类功法（如聚灵诀、轻身术）选择"辅助"。role 决定系统分配的功法效果类型：攻击→主动效果（造成伤害+附加效果），辅助→被动效果（增益/减益/触发）。功法名称明显是攻击手段时必须选"攻击"。
8. 功法命名硬约束：功法名称的最后一个字必须是"功""诀""术""法"之一，禁止使用其他字结尾。正确示例：长春功、聚灵诀、烈火术、御剑法；错误示例：碎石掌、混元劲、金钟罩。
...
5. NPC生成需要包含的信息：displayName（2-4字）、identity、currentStageGoal、longTermGoal、hobby、fear、personality、favorability、gender、age、linggen、realm、equippedSlots（最多4个法宝，其中至少1个为攻击性法宝如剑、刀等）、gongfaSlots（长度8，须含攻击类功法，每个含 bonus 和 system）、inventorySlots（最多12格）、hpPercent/mpPercent（血量/法力百分比，0-100整数，100为满状态）。
